        ...
        <View style={styles.container}>
          {chatsLoading ? (
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {conversations.map((c) => (
                <TouchableOpacity key={c.key} style={[styles.requestCard, {paddingVertical: 16}]} onPress={() => navigate('Chat', { request_id: c.request_id, receiver_id: c.other_id, other_party_name: c.other?.display_name || 'التاجر' })}>
                  <View style={{flexDirection: 'row-reverse', alignItems: 'center', gap: 12}}>
                    ...
                    <View style={{flex: 1}}>
                      <Text style={{color: '#FFF', fontWeight: 'bold', fontSize: 15, textAlign: 'right'}}>{c.other?.display_name || 'التاجر'}</Text>
